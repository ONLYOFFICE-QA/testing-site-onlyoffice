...
describe 'Desktop apps' do
  ...
  it '[Mobile][Android]Check "Get it on Google play" button' do
    ...
    expect(@mobile_app_page.check_opened_page_title).to eq(TestingSiteOnlyffice::SiteDownloadData::MOBILE_GOOGLE)
  end

  context 'IOS' do
    it '[Mobile][iOS]Check "Download on the app store" button' do
      @mobile_app_page.site_mobile_app_store
      expect(@mobile_app_page.check_opened_page_title).to eq(TestingSiteOnlyffice::SiteDownloadData::MOBILE_APP_STORE)
    end

    it '[Mobile][iOS]Check "Whats new" link' do
      @mobile_app_page.site_mobile_ios_whats_new
      expect(@mobile_app_page.check_opened_page_title).to eq(TestingSiteOnlyffice::SiteDownloadData::MOBILE_IOS_CHANGELOG)
    end
  end
