...
describe 'Connectors download' do
  ...
  TestingSiteOnlyffice::SiteDownloadData.connectors_list.each do |connector|
    it "[Site][DownloadConnectors]Check `#{connector}` 'Get on Github' link /download.aspx" do
      ...
      github_title = TestingSiteOnlyffice::SiteDownloadData.connectors_info[connector.to_s]['github_title']
    ...
    it "[Site][DownloadConnectors]Check `#{connector}`'Read instructions' link /download.aspx" do
      ...
      instruction_title = TestingSiteOnlyffice::SiteDownloadData.connectors_info[connector.to_s]['instruction_title']
