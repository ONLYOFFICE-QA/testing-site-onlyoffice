...
describe 'Buy Product Notification' do
  ...
  mail = IredMailHelper.new(username: 'avangate-buy-product@qamail.teamlab.info', password: 'SGc7ffk68T')
  ...
  it '[Site][PricingWorkspace] Buy ONLYOFFICE Workspace and check notify /workspace-enterprise-prices.aspx' do
    ...
    expect(mail.check_email_by_subject({ subject: TestingSiteOnlyffice::SiteNotificationData::PAYMENT_RECEIVED }, 300, true)).to be_truthy
    expect(mail.check_email_by_subject({ subject: TestingSiteOnlyffice::SiteNotificationData::PURCHASE_WORKSPACE }, 300, true)).to be_truthy
  end
  ...
  it '[Site][PricingDocsEnterprise] Buy Docs Enterprise Edition and check notify /docs-enterprise-prices.aspx' do
    ...
    avangate.submit_avangate_order_for_notification(email: mail.username)
    expect(mail.check_email_by_subject({ subject: TestingSiteOnlyffice::SiteNotificationData::PAYMENT_RECEIVED }, 300, true)).to be_truthy
    expect(mail.check_email_by_subject({ subject: TestingSiteOnlyffice::SiteNotificationData::PURCHASE_DOCS_ENTERPRISE }, 300,
                                       true)).to be_truthy
  end
  ...
  it '[Site][PricingDocsDeveloper] Buy Docs Developer Edition and check notify /developer-edition-prices.aspx' do
    ...
    avangate.submit_avangate_order_for_notification(email: mail.username)
    expect(mail.check_email_by_subject({ subject: TestingSiteOnlyffice::SiteNotificationData::PAYMENT_RECEIVED }, 300, true)).to be_truthy
    expect(mail.check_email_by_subject({ subject: TestingSiteOnlyffice::SiteNotificationData::PURCHASE_DOCS_DEVELOPER }, 300,
                                       true)).to be_truthy
  end
