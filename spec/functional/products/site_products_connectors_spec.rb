...
describe 'Site products connectors ' do
  ...
  TestingSiteOnlyoffice::SiteDownloadData.connectors_partners_list.each do |connector|
    describe connector.to_s do
      before do
        @products_connectors_partners_page = @products_connectors_page.switch_to_provided_by_partners
        @current_connector = @products_connectors_partners_page.partners_connector_block(connector)
      end
  ...
  TestingSiteOnlyoffice::SiteDownloadData.connectors_list.each do |connector|
    describe connector.to_s do
      before do
        @current_connector = @products_connectors_page.onlyoffice_connector_block(connector)
      end
