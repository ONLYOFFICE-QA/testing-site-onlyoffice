    ...
    SiteData.site_languages.each do |current_language|
      describe "Registration #{current_language}" do
        it "Registration from portal sign in page for #{current_language}" do
          admin = TestingSiteOnlyffice::SiteHelper.new.create_portal_change_language_site(@portal_creation_data, current_language)
          main_page, @test = TestingSiteOnlyffice::PortalHelper.new.init_instance(admin)
        ...
        it "Check welcome message for #{current_language}" do
          ...
          Timeout.timeout(5.minutes) do
            expect(TestingSiteOnlyffice::SiteNotificationHelper.check_site_notification(language: current_language,
                                                                                        module: 'WebStudio',
                                                                                        mail: mail,
        ...
        it "Check changing to #{current_language} language" do
          main_page, @test = TestingSiteOnlyffice::PortalHelper.new.open_page_teamlab_office
