...
module TestingSiteOnlyoffice
  ...
  class SiteDesktopConstructor
    ...
    def initialize(instance, installer)
      ...
      @download_xpath_x86 = "//a[contains(@id,'desktop_editors') and contains(@id,'#{@installer}') and contains(@id,'x86')]" if SiteDownloadData.desktop_download_list_type[:two_download_files].include?(@installer.to_sym)
    ...
    def fetch_download_xpath
      if SiteDownloadData.desktop_download_list_type[:two_download_files].include?(@installer.to_sym)
